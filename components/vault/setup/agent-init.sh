...
kubectl -n ${NAMESPACE} exec vault-helm-0 -- vault secrets enable -path=secret kv-v2

# Create policy for the sidecar
printf "[+] Creating policy for the Vault Sidecar...\n"
POLICY=$(cat ${POLICY_FILE})
kubectl -n ${NAMESPACE} exec vault-helm-0 -- /bin/sh -c "echo '$POLICY' > /tmp/policy.json"
kubectl -n ${NAMESPACE} exec vault-helm-0 -- vault policy write vault-agent /tmp/policy.json
kubectl -n ${NAMESPACE} exec vault-helm-0 -- vault write \
